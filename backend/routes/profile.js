...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const profile = await Profile.findOne({ user: req.loggedUser.id });
...
router.delete('/experience/:exp_id', jwt_auth, async (req, res) => {
  try {
    const foundProfile = await Profile.findOne({ user: req.loggedUser.id });
...
router.delete('/education/:edu_id', jwt_auth, async (req, res) => {
  try {
    const foundProfile = await Profile.findOne({ user: req.loggedUser.id });
